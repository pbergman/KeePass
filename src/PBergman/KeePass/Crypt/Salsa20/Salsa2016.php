 ...
 */
class Salsa2016 extends AbstractSalsa20
{
    /**
    ...
    {
        $iv = $iv = array_values(unpack('C8', $this->iv));

        if (strlen($this->key) === 32) {
            $key = array_values(unpack('C32', $this->key));
            $control = [0x7865, 0x6170, 0x646e, 0x3320, 0x2d32, 0x7962, 0x6574, 0x6b20]; // SIGMA
        } else {
            $key = array_values(unpack('C32', $this->key . $this->key));
