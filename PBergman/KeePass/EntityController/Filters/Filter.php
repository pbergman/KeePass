...
use PBergman\KeePass\Exceptions\InvalidComparisonOperatorException;
...
class Filter
{
    ...
    /** @var \PBergman\SharedMemory\Controller */
    protected $shm       = null;
     ...
     * @return $this
     *
     * @throws \PBergman\KeePass\Exceptions\InvalidComparisonOperatorException
        ...
        switch ($name) {
            ...
            default:
                foreach ( array_keys($this->entities) as $id ) {
                    ...
                    /** @var $entity \PBergman\KeePass\Entity\BaseEntity */
    ...
    public function search($search, $ci = false)
    {
        ...
        if (!empty($this->result)) {
            ...
            foreach ($this->result as $id) {
                ...
                /** @var \PBergman\KeePass\Entity\BaseEntity $entity */
                $entity     = $this->shm->varGet($id);
